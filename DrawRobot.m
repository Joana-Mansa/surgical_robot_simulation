...
% visualises a robotic manipulator
% includes the frames of reference at each joint
% and the links connecting them
% uses functions to compute the homogeneous transformations
% draw a robot in 3D with intermediate frames
% function DrawRobot(DH) 
% takes an input of the DH parameters
%
    n = size(DH,1);
    d = 0.03;

    % initialise homogeneous transformation matrices
    ...
    % visualisation setup
    hold on
    view([2 -2 2]) % sets the view angle for 3D plots
...
% with a specified parameter 'd'
% the function draws a cylindrical link between two points
